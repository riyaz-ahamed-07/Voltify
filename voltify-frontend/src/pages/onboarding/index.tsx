...
export default function Onboarding() {
  ...
  return (
    <LazyMotion features={domAnimation}>
      <div className="min-h-screen bg-background flex flex-col justify-center py-10 px-4 md:px-10 font-body text-on-surface">
        <div className="max-w-3xl w-full mx-auto">
          {/* Progress header */}
          <div className="text-center mb-8">
            <Link to="/" className="inline-flex items-center gap-2 mb-4">
              <div className="size-8 rounded-lg bg-surface border border-outline flex items-center justify-center">
                <Zap className="size-4 text-primary" />
              </div>
              <span className="font-display text-xl font-bold tracking-tight text-on-surface">VOLTIFY</span>
            </Link>
          ...
          <div className="flex justify-between max-w-md mx-auto text-[10px] uppercase font-semibold text-on-surface-variant tracking-wider">
            <span>Profile</span>
            <span>Billing</span>
        ...
        {/* Content Box */}
        <div className="bg-surface-container rounded-2xl p-8 border border-outline shadow-sm relative">
          <AnimatePresence mode="wait">
            {state.step === 1 && (
              <m.div
                ...
                exit={{ opacity: 0, x: -20 }}
              >
                <h3 className="font-display font-semibold text-lg text-on-surface mb-1">Step 1: Household Profile</h3>
                ...
                <form onSubmit={handleProfileSubmit(onProfileNext)} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Home Type */}
                    <div>
                      <label htmlFor="home_type" className="block text-xs font-semibold text-on-surface mb-2">Dwelling Type</label>
                      <select
                    ...
                    {/* Household Type */}
                    <div>
                      <label htmlFor="household_type" className="block text-xs font-semibold text-on-surface mb-2">Household Size</label>
                      <select
                    ...
                    <div className="md:col-span-2">
                      <label htmlFor="location" className="block text-xs font-semibold text-on-surface mb-2">Electricity Provider (DISCOM) Region</label>
                      <select
                        {...regProfile('location')}
                        id="location"
                        className="w-full px-4 py-2.5 bg-surface border border-outline rounded-lg text-on-surface text-sm focus:outline-none focus:border-primary transition-colors"
                  ...
                  <div className="flex justify-end pt-4">
                    <button
                      type="submit"
                      className="bg-on-surface text-surface px-6 py-2.5 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 hover:bg-on-surface/90"
                    >
            ...
            {state.step === 2 && (
              <m.div
                ...
                exit={{ opacity: 0, x: -20 }}
              >
                <h3 className="font-display font-semibold text-lg text-on-surface mb-1">Step 2: Utility Bill Details</h3>
                <p className="text-on-surface-variant text-sm mb-6">Upload a simulated electricity bill or enter your last billing figures manually.</p>

                {/* Dropzone */}
                <div
                  {...getRootProps()}
                  className={`border border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-300 mb-6 flex flex-col items-center ${
                    isDragActive
                ...
                <form onSubmit={handleBillSubmit(onBillNext)} className="space-y-6 mt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Bill amount */}
                    <div>
                      <label htmlFor="bill_amount" className="block text-xs font-semibold text-on-surface mb-2">Recent Bill Amount (₹)</label>
                      <input
                  ...
                  <div className="flex justify-between pt-4">
                    <button
                      ...
                      className="border border-outline text-on-surface-variant px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-surface transition-all"
                    >
                      Back
                    </button>
                    <button
            ...
            {state.step === 3 && (
              <m.div
                ...
                exit={{ opacity: 0, x: -20 }}
              >
                <h3 className="font-display font-semibold text-lg text-on-surface mb-1">Step 3: Home Appliance Calibration</h3>
                <p className="text-on-surface-variant text-sm mb-6">Select key household appliances and adjust their estimated daily active hours.</p>
                ...
                <div className="space-y-4 max-h-[380px] overflow-y-auto pr-2">
                  {Object.keys(DEFAULT_APPLIANCES).map((key) => {
                    ...
                    return (
                      <div
                        key={key}
                        className={`p-4 rounded-xl border transition-all duration-350 flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                          isSelected
                            ? 'bg-primary/5 border-primary/20'
                            : 'bg-surface border-outline opacity-70 hover:opacity-100'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-on-surface-variant mt-0.5">Power Rating: {app.power_kw} kW</p>
                          </div>
                        </div>
            ...
            {state.step === 4 && currentCalc && state.billData && state.profileData && (
              ...
              >
                <div className="text-center">
                  <div className="size-12 bg-tertiary/10 rounded-full border border-tertiary/30 flex items-center justify-center mx-auto mb-3">
                    <ShieldCheck className="size-6 text-tertiary" />
                  </div>
                  <h3 className="font-display font-semibold text-lg text-on-surface">Calibration Complete</h3>
                  <p className="text-on-surface-variant text-sm mt-1">Our estimates successfully match your utility provider bill details.</p>
                </div>

                {/* Score Stats */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-surface border border-outline p-4 rounded-xl text-center">
                    <span className="text-xs font-semibold text-on-surface-variant block mb-1">Utility Bill Load</span>
                    <span className="font-semibold text-lg text-on-surface">{formatUnits(state.billData.units)}</span>
                    <span className="block text-xs text-on-surface-variant mt-0.5">({formatCurrency(state.billData.bill_amount)})</span>
                  </div>

                  <div className="bg-surface border border-outline p-4 rounded-xl text-center">
                    <span className="text-xs font-semibold text-on-surface-variant block mb-1">Estimated Baseline</span>
                    <span className="font-semibold text-lg text-primary">{formatUnits(currentCalc.estKwh)}</span>
                    <span className="block text-xs text-on-surface-variant mt-0.5">({formatCurrency(Math.round(currentCalc.estKwh * currentCalc.rate))})</span>
                  </div>

                  <div className="bg-surface border border-outline p-4 rounded-xl text-center">
                    <span className="text-xs font-semibold text-on-surface-variant block mb-1">Calibration Accuracy</span>
                    <span className={`font-semibold text-lg ${
                      currentCalc.accuracy >= 80 ? 'text-tertiary' : 'text-error'
                    }`}>{currentCalc.accuracy}%</span>
                    <span className="block text-xs text-on-surface-variant mt-0.5">Ready for Analysis</span>
                  </div>
                </div>

                {/* Math Disaggregation breakdown text list */}
                <div className="bg-surface border border-outline p-5 rounded-xl space-y-3">
                  <h4 className="font-semibold text-xs text-on-surface mb-2">Estimated Appliance Share</h4>
                    ...
                    {currentCalc.appliances.slice(0, 4).map((app) => {
                      ...
                      return (
                        <div key={app.id} className="flex justify-between items-center text-sm">
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-on-surface-variant">{state.applianceHours[app.id]} hrs/day</span>
                            <span className="font-semibold text-on-surface">{sharePct}%</span>
                          </div>
                        </div>
                      );
                    })}
                    {currentCalc.appliances.length > 4 && (
                      <p className="text-center text-xs text-on-surface-variant italic">+ {currentCalc.appliances.length - 4} more appliances</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-between pt-4">
                  ...
                  <button
                    ...
                    className="bg-primary hover:bg-primary/90 text-surface px-6 py-2.5 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 shadow-sm"
                  >
                    Complete Onboarding <Play className="size-4 fill-current text-surface" />
                  </button>
                </div>
              </m.div>
            )}
